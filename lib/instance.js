...
var InstanceProcessObserver = function (process, instance) {
	...
	var onStarted = function () {
		startLogging();
	    if (instance.snapshot && instance.snapshot.config.resolvePort) {
			...
			instance._activePort = instance.snapshot.port || 0;
			instance.emit('started', instance);
	    }
	};
	...
	var onStart = function () {
        ...
        instance.emit('command.exec', process.command + ' ' + (process.args || []).join(' '));
	};
...
Instance.prototype.start = function () {
	if (this.state !== STATE_RUNNING) {
		return this.process.start().bind(this).then(function () {
			return this;
		});
	}
