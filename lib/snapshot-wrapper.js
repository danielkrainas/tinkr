var _ = require('lodash');
var exec = require('child_process').exec;
var path = require('path');
var AdmZip = require('adm-zip');
var fse = require('fs-extra');
var async = require('async');
var util = require('util');
var EventEmitter = require('events').EventEmitter;

var config = require('../config');


var SNAPSHOTS_DIR = config.snapshotFolder;

var clearOrCreateDirectory = function (dir, callback) {
    fse.ensureDirSync(dir);
    fse.readdir(dir, function (err, files) {
        if (err) {
            return callback(err);
        }

        var work = [];
        files.forEach(function (file) {
            work.push(function (next) {
                fse.remove(path.join(dir, file), function (err) {
                    if (err) {
                        return next(err);
                    }

                    console.log('-removed: ' + file);
                    next();
                });
            });
        });

        async.series(work, function (err) {
            if (err) {
                return callback(err);
            }

            console.log('-cleaned');
            callback();
        });
    });
};

var defaultConfig = {
    project: null,
    version: null,
    proxy: null,
    install: [],
    command: null,
    args: [],
    env: []
};

var SnapshotWrapper = function (settings) {
    var snapshot = this;
    settings = _.merge({}, defaultConfig, settings);
    var installedPath = null;

    Object.defineProperties(this, {
        uid: {
            get: function () {
                return settings.project + '#' + settings.version;
            }
        },

        project: {
            get: function () {
                return settings.project;
            }
        },

        version: {
            get: function () {
                return settings.version;
            }
        },

        port: {
            get: function () {
                return settings.port;
            }
        },

        installed: {
            get: function () {
                return !!installedPath;
            }
        },

        config: {
            writable: false,
            value: settings
        },

        install: {
            writable: false,
            value: function (callback) {
                if (snapshot.installed) {
                    return;
                }

                console.log('installing');
                var home = installedPath = path.join(config.projectFolder, snapshot.project);
                clearOrCreateDirectory(home, function (err) {
                    if (err) {
                        return callback(err);
                    }

                    var zip = new AdmZip(path.join(SNAPSHOTS_DIR, snapshot.project + '-' + snapshot.version + '.zip'));
                    zip.extractAllTo(home, true);
                    if (settings.install) {
                        if (typeof settings.install === 'string') {
                            settings.install = [settings.install];
                        }

                        var work = [];
                        settings.install.forEach(function (command) {
                            work.push(function (done) {
                                console.log('-install command: ' + command);
                                exec(command, {
                                    cwd: home,
                                    env: process.env
                                }, done);
                            });
                        });

                        async.series(work, function (err) {
                            if (err) {
                                return callback(err);
                            }

                            console.log('installed');
                            callback();
                        });
                    } else {
                        console.log('installed');
                        callback();
                    }
                });
            }
        },

        uninstall: {
            writable: false,
            value: function (callback) {
                if (snapshot.installed) {
                    console.log('uninstalling');
                    clearOrCreateDirectory(installedPath, function (err) {
                        if (err) {
                            return callback(err);
                        }

                        installedPath = null;
                        console.log('uninstalled');
                        callback();
                    });
                }
            }
        },

        store: {
            writable: false,
            value: function (file, callback) {
                var newPath = path.join(SNAPSHOTS_DIR, settings.project + '-' + settings.version + '.' + file.extension);
                fse.move(file.path, newPath, function (err) {
                    console.log('stored binary: ' + newPath);
                    callback(err);
                });
            }
        },

        toString: {
            writable: false,
            value: function () {
                return snapshot.uid;
            }
        },

        setEnv: {
            writable: false,
            value: function (key, value) {
                if (!key) {
                    return;
                }

                settings.env = settings.env || {};
                if (!value) {
                    delete settings.env[key];
                } else {
                    settings.env[key] = value;
                }
            }
        }
    });
};

util.inherits(SnapshotWrapper, EventEmitter);

module.exports = exports = SnapshotWrapper;
