...
ProjectList.prototype.getByDomain = function (hostname) {
    // todo: fix
    /*var projects = _.values(this.loaded);
    for (var i = 0; i < projects.length; i++) {
        var domains = projects[i].domains;
        var found = domains.any(function (domain) {
            return domain == hostname || wildcard(domain, hostname);
        });
...
ProjectList.prototype.remove = function (name, callback) {
    ...
    this.collection.remove({ name: project.name }).then(function () {
        ...
        // todo: remove from domain map as well
