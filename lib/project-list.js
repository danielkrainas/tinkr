...
var _add = function (list, descriptor, callback) {
    ...
    project.init(function () {
        ...
        if (project.hosting && project.hosting.relay) {
            var domains = project.hosting.relay.domain;
            if (!Array.isArray(domains)) {
                domains = [domains];
            }

            domains.forEach(function (domain) {
                if (!list.domainMap.hasOwnProperty(domain)) {
                    list.domainMap[domain] = project;
                }
            });
        }
...
ProjectList.prototype.remove = function (name, callback) {
    ...
    this.collection.remove({ name: project.name }).then(function () {
        project.dispose();
        delete list.loaded[project.name];
        if (project.hosting && project.hosting.relay) {
            var domains = project.hosting.relay.domain;
            if (!Array.isArray(domains)) {
                domains = [domains];
